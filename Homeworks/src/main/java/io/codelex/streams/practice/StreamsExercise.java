...
public class StreamsExercise {
    public static List<Integer> returnSquareRoot(List<Integer> numbers) {
        return numbers.stream().map(n -> (int) Math.round(Math.sqrt(n))).collect(Collectors.toList());
    }
    ...
    public static List<String> getFirstNames(List<String> names) {
        return names.stream().map(name -> name.contains(" ") ? name.substring(0, name.indexOf(" ")) : name).toList();
    }

    public static List<String> getDistinctLetters(List<String> names) {
        return names.stream().flatMap(name -> name.chars().mapToObj(c -> String.valueOf((char) c)))
                .distinct().toList();
    }
    ...
    public static double getAverageAge(List<User> users) {
        return users.stream().mapToInt(User::getAge).average().orElseThrow(UnsupportedOperationException::new);
    }

    public static Integer getMaxAge(List<User> users) {
        return users.stream().mapToInt(User::getAge).max().orElseThrow(UnsupportedOperationException::new);
    }

    public static Integer getMinAge(List<User> users) {
        return users.stream().mapToInt(User::getAge).min().orElseThrow(UnsupportedOperationException::new);
    }
    ...
    public static List<Integer> generate10RandomNumbers() {
        return IntStream.generate(() -> 1 + (int) (Math.random() * 100))
                .limit(10)
                .boxed()
                .collect(Collectors.toList());
    }
    ...
    public static User findOldest(List<User> users) {
        return users.stream().max(Comparator.comparing(User::getAge)).orElseThrow(UnsupportedOperationException::new);
    }
